            ...
            <div>
                <div>
                    <div className="grid place-items-center p-10">
                        <input 
                            className="border w-40 focus:outline-none focus:ring focus:w-96 duration-500 rounded-lg p-2"
                            type="search" 
                    ...
                    <div>
                        <div className="grid place-items-center">
                            { !weatherData ? (
                                <div className="text-gray-600">Enter City and press Enter to get information</div>
                            ) : weatherData.main ? (
                                <div className="border p-5 rounded-xl shadow-md shadow-current">
                                    <div className="grid place-items-center text-xl font-semibold leading-7">
                                        <div className="weather-icon w-60">
                                            <img src={`http://openweathermap.org/img/wn/${weatherData.weather[0].icon}@4x.png`} alt="weather-icon" className="w-full" />
                                        </div>
                                        <div className="weather-temperature">{Math.round(weatherData.main.temp)}°C</div>
                                        <div className="city">{weatherData.name}</div>
                                        <div className="mini-description">{weatherData.weather[0].description}</div>
                                    </div>
                                </div>
                            ) : (
